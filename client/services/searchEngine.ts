...
export class ProductSearchEngine {
      ...
      if (product.variantSummaries && product.variantSummaries.length > 0) {
        for (let i = 0; i < product.variantSummaries.length; i++) {
          ...
          if (variantSku && variantSku.length > 0) {
            ...
            console.log(`[SearchEngine] Indexed variant SKU: "${variantSku}" -> "${product.name}" / "${variant.name}"`);
          }
  ...
  search(query: string, options: SearchOptions = {}): SearchResult[] {
    ...
    if (includeVariants) {
      ...
      if (variantBySku) {
        ...
        console.log(`[SearchEngine] Found variant by exact SKU: "${variant.name}"`);
      ...
      if (variantByBarcode) {
        ...
        const variant = product.variantSummaries![variantIndex];
        console.log(`[SearchEngine] Found variant by exact barcode: "${variant.name}"`);
      ...
      if (/^\d+$/.test(normalizedQuery)) {
        ...
        console.log(`[SearchEngine] Trying numeric: "${normalizedQuery}" -> "${withoutZeros}"`);
        
        // Avval asosiy mahsulotni tekshirish (raqamli SKU)
        const productByNumeric = this.productSkuMap.get(withoutZeros);
        if (productByNumeric) {
          console.log(`[SearchEngine] Found product by numeric SKU: "${productByNumeric.name}"`);
        ...
        if (variantByNumeric) {
          ...
          const variant = product.variantSummaries![variantIndex];
          console.log(`[SearchEngine] Found variant by numeric: "${variant.name}"`);
          results.push(this.createVariantResult(product, variant, variantIndex, 100, 'sku'));
          this.queryCache.set(cacheKey, results);
          return results;
        }
        ...
        for (let zeros = 1; zeros <= 4; zeros++) {
          ...
          if (productByPadded) {
            console.log(`[SearchEngine] Found product by padded SKU: "${paddedQuery}" -> "${productByPadded.name}"`);
        ...
        for (let zeros = 1; zeros <= 4; zeros++) {
          ...
          if (variantByPadded) {
            ...
            const variant = product.variantSummaries![variantIndex];
            console.log(`[SearchEngine] Found variant by padded: "${paddedQuery}" -> "${variant.name}"`);
      ...
      if (results.length > 0) {
        console.log(`[SearchEngine] Found ${results.length} variants by partial match`);
    ...
    if (includeVariants && normalizedQuery.length >= 2) {
      ...
      if (variantsByName && variantsByName.length > 0) {
        console.log(`[SearchEngine] Found ${variantsByName.length} variants by exact name: "${normalizedQuery}"`);
      ...
      if (partialNameMatches.length > 0) {
        console.log(`[SearchEngine] Found ${partialNameMatches.length} variants by partial name match`);
    ...
    if (productBySku) {
      console.log(`[SearchEngine] Found product by SKU: "${productBySku.name}"`);
  ...
  findByCodeWithVariant(code: string): { product: OfflineProduct; variantIndex?: number } | undefined {
    ...
    console.log(`[SearchEngine] findByCodeWithVariant: "${key}"`);
    
    // 1. To'liq moslik - Variant SKU (eng yuqori prioritet)
    const variantBySku = this.variantSkuMap.get(key);
    if (variantBySku) {
      console.log(`[SearchEngine] Found variant by exact SKU: "${key}"`);
      return variantBySku;
    }
    ...
    if (variantByBarcode) {
      console.log(`[SearchEngine] Found variant by exact barcode: "${key}"`);
    ...
    if (/^\d+$/.test(key)) {
      ...
      console.log(`[SearchEngine] Trying numeric search: "${key}" -> "${withoutLeadingZeros}"`);
      ...
      if (withoutLeadingZeros !== key) {
        ...
        if (variantByNumeric) {
          console.log(`[SearchEngine] Found variant by numeric (no zeros): "${withoutLeadingZeros}"`);
      ...
      for (let totalLength = withoutLeadingZeros.length + 1; totalLength <= 6; totalLength++) {
        ...
        if (paddedKey !== key) { // O'zimizni qayta qidirmaslik
          console.log(`[SearchEngine] Trying padded: "${paddedKey}"`);
          const variantByPadded = this.variantSkuMap.get(paddedKey);
          if (variantByPadded) {
            console.log(`[SearchEngine] Found variant by padded: "${paddedKey}"`);
    ...
    if (/^\d+$/.test(key)) {
      ...
      if (withoutLeadingZeros !== key) {
        ...
        if (productByNumeric) {
          console.log(`[SearchEngine] Found product by numeric SKU: "${withoutLeadingZeros}"`);
      ...
      for (let totalLength = withoutLeadingZeros.length + 1; totalLength <= 6; totalLength++) {
        ...
        if (paddedKey !== key) {
          ...
          if (productByPadded) {
            console.log(`[SearchEngine] Found product by padded SKU: "${paddedKey}"`);
    ...
    console.log(`[SearchEngine] Not found: "${key}"`);
    return undefined;
  }
