...
export function useBarcodeScanner({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // HAR BIR tugmani log qilish
      console.log('[BarcodeScanner] KEY:', e.key, 'code:', e.code);
      ...
      if (isInputFocused) {
        console.log('[BarcodeScanner] Input focused, ignoring');
        return;
      }

      // Modifier keys - o'tkazib yuborish
      if (e.ctrlKey || e.altKey || e.metaKey) {
        console.log('[BarcodeScanner] Modifier key, ignoring');
        return;
      }
