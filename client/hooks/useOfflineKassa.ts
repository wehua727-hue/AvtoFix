...
export function useOfflineKassa(userId: string, userPhone?: string, defectiveCounts?: Map<string, number>): UseOfflineKassaReturn {
  ...
  useEffect(() => {
    ...
    // Product update listener - ota mahsulot o'chirilganda cache yangilash
    const handleProductUpdate = (event: CustomEvent) => {
      const { productId, action } = event.detail;
      
      if (action === 'variant-promoted') {
        console.log('[useOfflineKassa] Product variant promoted, refreshing cache for:', productId);
        // Cache ni yangilash - keyingi qidiruv fresh ma'lumotlar bilan bo'ladi
        refreshCache();
      }
    };
  ...
  }, [userId, fetchProductsFromMongoDB, refreshCache]);
  ...
  const addToCart = useCallback((product: OfflineProduct, isRefundMode: boolean = false) => {
    setItems(prev => {
      ...
      const newItem = {
        ...
        sku: product.sku,
        customId: product.customId, // ✅ YANGI: Custom ID
  ...
  const searchBySkuWithVariant = useCallback(async (code: string): Promise<{ product: OfflineProduct; variantIndex?: number } | undefined> => {
    ...
    try {
      ...
      // ✅ 0. YANGI: CustomId bo'yicha qidirish (eng birinchi)
      const normalizedCodeUpper = code.trim().toUpperCase();
      console.log('[searchBySkuWithVariant] Searching for customId:', normalizedCodeUpper);

      for (const product of products) {
        // Debug: Mahsulot customId ni ko'rsatish
        if ((product as any).customId) {
          console.log('[searchBySkuWithVariant] Product customId:', (product as any).customId, 'Name:', product.name);
        }

        if ((product as any).customId?.toUpperCase() === normalizedCodeUpper) {
          console.log('[searchBySkuWithVariant] Found product by customId:', normalizedCodeUpper);
          return { product, variantIndex: undefined };
        }

        // ✅ YANGI: Variant customId bo'yicha qidirish
        if (product.variantSummaries && product.variantSummaries.length > 0) {
          for (let i = 0; i < product.variantSummaries.length; i++) {
            const variant = product.variantSummaries[i];

            // Debug: Xil customId ni ko'rsatish
            if ((variant as any).customId) {
              console.log('[searchBySkuWithVariant] Variant customId:', (variant as any).customId, 'Name:', variant.name);
            }

            if ((variant as any).customId?.toUpperCase() === normalizedCodeUpper) {
              console.log('[searchBySkuWithVariant] Found variant by customId:', normalizedCodeUpper);
    ...
    try {
      ...
      const sale: OfflineSale = {
        ...
        items: items.map(item => ({
          ...
          name: item.name,
          sku: item.sku,
          customId: item.customId, // ✅ YANGI: Custom ID
