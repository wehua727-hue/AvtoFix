...
export function useOfflineKassa(userId: string, userPhone?: string): UseOfflineKassaReturn {
  ...
  const completeSale = useCallback(async (
    ...
    try {
      ...
      // ONLINE: To'g'ridan-to'g'ri MongoDB ga saqlash
      // OFFLINE: IndexedDB ga saqlash (keyinroq sinxronlanadi)
      if (isOnline) {
        console.log(`[useOfflineKassa] ONLINE - Saving sale directly to MongoDB...`);
        try {
          const response = await fetch(`${apiBaseUrl}/api/sales/offline-sync`, {
            method: 'POST',
          ...
          if (response.ok) {
            ...
            if (result.syncedIds?.includes(sale.id)) {
              console.log(`[useOfflineKassa] ✅ Sale saved to MongoDB successfully:`, sale.id);
              sale.synced = true;
              // IndexedDB ga saqlamaymiz - faqat MongoDB da
            } else {
              console.error(`[useOfflineKassa] ❌ MongoDB sync failed, saving to IndexedDB as fallback`);
              await saveOfflineSale(sale);
            }
          } else {
            console.error(`[useOfflineKassa] ❌ Server error, saving to IndexedDB as fallback`);
            await saveOfflineSale(sale);
          }
        } catch (networkError) {
          console.error(`[useOfflineKassa] ❌ Network error, saving to IndexedDB:`, networkError);
          await saveOfflineSale(sale);
        }
      } else {
        // OFFLINE - IndexedDB ga saqlash
        console.log(`[useOfflineKassa] OFFLINE - Saving sale to IndexedDB:`, { id: sale.id, itemsCount: sale.items.length });
        await saveOfflineSale(sale);
        
        // Verify sale was saved
        await new Promise(resolve => setTimeout(resolve, 100));
        const savedSale = await offlineDB.offlineSales.get(sale.id);
        console.log(`[useOfflineKassa] Sale saved to IndexedDB:`, { id: savedSale?.id, exists: !!savedSale });
      }
      
      // Update pending count (faqat IndexedDB dagi sinxronlanmagan savdolar)
      const unsyncedSales = await getUnsyncedSales();
      setPendingSalesCount(unsyncedSales.length);
      console.log(`[useOfflineKassa] Pending sales count:`, unsyncedSales.length);
