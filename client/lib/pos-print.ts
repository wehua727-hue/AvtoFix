...
function buildReceiptData(receipt: ReceiptData): Uint8Array {
  ...
  const totalValue = `${receipt.type === 'refund' ? '-' : ''}${receipt.total.toLocaleString()} so'm`;
...
export async function printTestReceipt(printerId: string | null): Promise<boolean> {
  const testReceipt: ReceiptData = {
    type: 'sale',
    items: [
      { name: 'Test mahsulot 1', quantity: 2, price: 15000 },
      { name: 'Test mahsulot 2', quantity: 1, price: 25000, discount: 10 },
    ],
    total: 52500,
    paymentType: 'Naqd',
    date: new Date(),
...
function buildLabelData(label: LabelData): Uint8Array {
  ...
  // Product name - bold
  addBytes(ESCPOS.BOLD_ON);
  const name = label.name.length > maxNameLength 
