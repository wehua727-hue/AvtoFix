...
import { getDefectiveProducts, DefectiveProduct, clearDefectiveProducts } from '@/db/offlineDB';
...
export default function Stats() {
  ...
  const [clearDailyDialogOpen, setClearDailyDialogOpen] = useState(false);
  const [clearWeeklyDialogOpen, setClearWeeklyDialogOpen] = useState(false);
  ...
  const handleClearDailyStats = async () => {
    if (!user || (user.role !== 'egasi' && user.role !== 'admin')) return;
    
    try {
      const res = await fetch(`${API_BASE}/api/sales/clear/daily`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user.id }),
      });
      
      if (!res.ok) throw new Error('Failed to clear daily stats');
      ...
      setClearDailyDialogOpen(false);
      toast.success('Kunlik statistika tozalandi');
      
      // Ma\'lumotlarni qayta yuklash
      if (user?.id) loadStats(user.id);
    } catch (error) {
      console.error('Kunlik statistikani tozalashda xato:', error);
      toast.error('Xatolik yuz berdi');
    }
  };

  // Haftalik statistikani tozalash
  const handleClearWeeklyStats = async () => {
    if (!user || (user.role !== 'egasi' && user.role !== 'admin')) return;
    
    try {
      const res = await fetch(`${API_BASE}/api/sales/clear/weekly`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user.id }),
      });
      
      if (!res.ok) throw new Error('Failed to clear weekly stats');
      ...
      setClearWeeklyDialogOpen(false);
      toast.success('Haftalik statistika tozalandi');
      
      // Ma\'lumotlarni qayta yuklash
      if (user?.id) loadStats(user.id);
    } catch (error) {
      console.error('Haftalik statistikani tozalashda xato:', error);
      toast.error('Xatolik yuz berdi');
    }
  };
  ...
  const loadStats = async (userId: string) => {
    setLoading(true);
    try {
      // Yangi API endpoint - qaytarilganlarni hisobga oladi
      const res = await fetch(`${API_BASE}/api/sales/stats/daily?userId=${encodeURIComponent(userId)}`);
      const data = await res.json();

      if (res.ok && data.success) {
        setDailyStats(data.stats);
      } else {
        // Xatolik bo'lsa, bo'sh statistika
        setDailyStats({
          netQuantity: 0,
          netRevenue: 0,
          totalOrders: 0,
      ...
      // Haftalik statistika uchun eski API dan foydalanish (hozircha)
      const weeklyRes = await fetch(`${API_BASE}/api/sales/offline/stats?userId=${encodeURIComponent(userId)}`);
      const weeklyData = await weeklyRes.json();

      if (weeklyRes.ok && weeklyData.success) {
        const serverWeekly = weeklyData.weekly || {};
        setWeeklyStats({
  ...
  return (
    <>
            ...
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="space-y-4 sm:space-y-6">
              ...
              <TabsContent value="daily" className="space-y-4 sm:space-y-6">
                <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
                  <StatCard title="Daromad" value={formatCurrency(dailyStats.netRevenue)} icon={DollarSign} color="from-emerald-500 to-teal-600" />
                  <StatCard title="Sotilgan" value={dailyStats.netQuantity} icon={Package} color="from-blue-500 to-indigo-600" />
                  ...
                  <Card className={`${isLight ? 'bg-white border-slate-200' : 'bg-slate-800/50 border-slate-700/50'} shadow-lg backdrop-blur-sm`}>
                    <CardHeader className="pb-2 sm:pb-4">
                      <div className="flex items-center justify-between">
                        ...
                        {(user?.role === 'egasi' || user?.role === 'admin') && (
                          <Button
                            variant="destructive"
                            size="sm"
                            onClick={() => setClearDailyDialogOpen(true)}
                    ...
                    <CardContent className="p-3 sm:p-6">
                      {dailyStats.products.length > 0 ? (
                        <div className="space-y-2 sm:space-y-3">
                          {dailyStats.products.map((product, index) => (
                            <motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} whileHover={{ scale: 1.01 }}
                              className={`p-3 sm:p-4 rounded-xl ${isLight ? 'bg-slate-50 hover:bg-slate-100 border border-slate-200' : 'bg-slate-900/50 hover:bg-slate-900/70 border border-slate-700/50'} transition-all`}>
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2 sm:gap-4">
                                  ...
                                  <div>
                                    <p className={`font-semibold text-sm sm:text-base ${isLight ? 'text-slate-800' : 'text-white'}`}>{product.productName}</p>
                                    <p className={`text-xs sm:text-sm ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>{product.netQuantity} dona</p>
                                  </div>
                                </div>
                                <Badge className="bg-gradient-to-r from-green-500 to-emerald-500 text-white border-0 text-xs sm:text-sm">{formatCurrency(product.netRevenue)}</Badge>
                              </div>
                            </motion.div>
                          ))}
                        </div>
                    ...
                    <Card className={`${isLight ? 'bg-white border-slate-200' : 'bg-slate-800/50 border-slate-700/50'} shadow-lg`}>
                      ...
                      <CardContent className="p-2 sm:p-6">
                        <ResponsiveContainer width="100%" height={250}>
                          <PieChart>
                            <Pie 
                              data={dailyStats.products} 
                              cx="50%" 
                              cy="50%" 
                              labelLine={false} 
                              label={(entry: any) => `${entry.productName}: ${entry.netQuantity}`}
                              outerRadius={80} 
                              fill="#8884d8" 
                              dataKey="netQuantity"
                            >
                              {dailyStats.products.map((_, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                            </Pie>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {(user?.role === 'egasi' || user?.role === 'admin') && (
                          <Button
                            variant="destructive"
                            size="sm"
                            onClick={() => setClearWeeklyDialogOpen(true)}
                            className="flex items-center gap-2"
                          >
                            <Trash2 className="w-4 h-4" />
                            Tozalash
      ...
      {/* Kunlik statistikani tozalash dialogi */}
      <AlertDialog open={clearDailyDialogOpen} onOpenChange={setClearDailyDialogOpen}>
        <AlertDialogContent className={`${isLight ? 'bg-white' : 'bg-slate-900 border-slate-700'}`}>
          <AlertDialogHeader>
            <AlertDialogTitle className={`flex items-center gap-2 ${isLight ? 'text-slate-800' : 'text-white'}`}>
              <Trash2 className="w-5 h-5 text-red-500" />
            ...
            <AlertDialogDescription className={isLight ? 'text-slate-600' : 'text-slate-400'}>
              Bugungi barcha sotuvlar va statistika o'chiriladi. Bu amalni qaytarib bo'lmaydi. Davom etasizmi?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className={isLight ? '' : 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700'}>
      ...
      {/* Haftalik statistikani tozalash dialogi */}
      <AlertDialog open={clearWeeklyDialogOpen} onOpenChange={setClearWeeklyDialogOpen}>
        <AlertDialogContent className={`${isLight ? 'bg-white' : 'bg-slate-900 border-slate-700'}`}>
          <AlertDialogHeader>
            <AlertDialogTitle className={`flex items-center gap-2 ${isLight ? 'text-slate-800' : 'text-white'}`}>
              <Trash2 className="w-5 h-5 text-red-500" />
            ...
            <AlertDialogDescription className={isLight ? 'text-slate-600' : 'text-slate-400'}>
              Haftalik barcha sotuvlar va statistika o'chiriladi. Bu amalni qaytarib bo'lmaydi. Davom etasizmi?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className={isLight ? '' : 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700'}>
            ...
            <AlertDialogAction
              onClick={handleClearWeeklyStats}
              className="bg-red-600 hover:bg-red-700 text-white"
            >
              Ha, tozalash
            </AlertDialogAction>
