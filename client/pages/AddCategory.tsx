...
export default function AddCategory() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!categoryName.trim() || !selectedStoreId) return;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* Main Content */}
      <div
        className={`pt-28 sm:pt-32 lg:pt-36 pb-12 px-4 sm:px-6 lg:px-8 relative z-10 transition-all duration-300 ${
          ...
          <div className="mb-8 flex flex-col gap-3">
            <div className="flex items-center justify-between gap-6">
              ...
              <button
                type="button"
                onClick={() => navigate(-1)}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/30 text-sm font-semibold text-white bg-white/5 backdrop-blur transition hover:border-white hover:bg-white/10"
              >
                <ArrowLeft className="w-4 h-4" />
                Orqaga
              </button>
            </div>
            <div className="h-1 w-28 bg-gradient-to-r from-transparent via-red-500 to-transparent" />
          </div>

          {/* Hierarchical categories list */}
          <div className="mb-8 bg-gradient-to-br from-slate-900/80 via-slate-900 to-slate-900/30 rounded-[32px] border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,0.65)] backdrop-blur p-6">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                {selectedParent ? (
                  <>
                    <div className="flex items-center gap-2 text-sm text-gray-400">
                      <span className="px-3 py-1 rounded-full border border-red-600/40 bg-red-900/10 text-xs">
                        Kategoriyalar
                      </span>
                      <span className="text-red-400">/</span>
                      <span className="text-red-300 font-medium">{selectedParent.name}</span>
                    </div>
                    <h2 className="text-xl font-semibold text-white">
                      {selectedParent.name}
                    </h2>
                    <p className="text-xs text-gray-400">
                      Tanlangan kategoriya ichidagi kichik kategoriyalar
                    </p>
                  </>
                ) : (
                  <>
              ...
              {selectedParent && (
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-red-600/40 text-xs text-gray-100 hover:bg-red-600/15 transition"
                >
                  <ArrowLeft className="w-4 h-4" />
                  Ortga
                </button>
