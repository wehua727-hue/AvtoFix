...
import { Plus, RefreshCw, CheckCircle, XCircle, Edit, Trash2, Wallet, Phone, Calendar, FileText, Ban, AlertTriangle, Search, X, Sparkles, Clock, MessageSquare, Download, Upload } from 'lucide-react';
...
import { DebtExcelImportModal } from '@/components/Debts/DebtExcelImportModal';
...
export default function Debts() {
      ...
      import('xlsx').then((XLSX) => {
        ...
        const worksheetData = [
          ...
          ...filteredDebts.map((debt, index) => [
            ...
            debt.status === 'pending' ? 'Kutilmoqda' :
            debt.status === 'paid' ? 'To\'langan' :
            debt.status === 'overdue' ? 'Muddati o\'tgan' :
            debt.status === 'unpaid' ? 'Qora ro\'yxat' : debt.status,
            debt.description || ''
          ])
        ];
        ...
        const fileName = `Qarzlar_${currentDate}.xlsx`;
        ...
        toast({
          title: 'Muvaffaqiyatli',
          description: `${filteredDebts.length} ta qarz Excel faylga eksport qilindi`
        });
      });
    } catch (error) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.endsWith('.xlsx') && !file.name.endsWith('.xls')) {
      toast({
        title: 'Xatolik',
        description: 'Faqat Excel fayllarini (.xlsx, .xls) yuklash mumkin',
        variant: 'destructive'
      });
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        import('xlsx').then((XLSX) => {
          ...
          const parsedData = rows.map((row: any[], index: number) => ({
            id: index + 1,
            creditor: row[1] || '',
            phone: row[2] || '',
        ...
        toast({
          title: 'Xatolik',
          description: 'Excel faylni o\'qishda xatolik yuz berdi',
          variant: 'destructive'
        });
      }
    };
  ...
  const handleImportDebts = async () => {
    if (excelData.length === 0) {
      toast({
        title: 'Xatolik',
        description: 'Import qilish uchun ma\'lumot topilmadi',
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      ...
      for (const debtData of excelData) {
        try {
          ...
          if (debtData.debtDate) {
            const parsedDebtDate = new Date(debtData.debtDate);
          ...
          if (debtData.dueDate) {
            const parsedDueDate = new Date(debtData.dueDate);
      ...
      toast({
        ...
        description: `${successCount} ta qarz muvaffaqiyatli qo'shildi${errorCount > 0 ? `, ${errorCount} ta xatolik` : ''}`
      });
      ...
      toast({
        title: 'Xatolik',
        description: 'Import jarayonida xatolik yuz berdi',
        variant: 'destructive'
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 dark:from-gray-900 dark:via-slate-900 dark:to-gray-900">
          ...
          <div className="flex items-center gap-2">
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button 
                ...
                className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white text-sm px-4 py-2 h-auto shadow-lg shadow-green-900/30 border-0"
              >
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button 
                ...
                className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-sm px-4 py-2 h-auto shadow-lg shadow-blue-900/30 border-0"
              >
      ...
      <main className={`pt-14 sm:pt-16 lg:pt-18 transition-all duration-300 ${sidebarCollapsed ? 'lg:pl-20' : 'lg:pl-80'}`}>
        ...
        {/* Excel Import Modal */}
        <DebtExcelImportModal
          isOpen={showExcelImport}
          onClose={() => setShowExcelImport(false)}
          onImportComplete={loadDebts}
        />
      </main>
    </div>
  );
