/**
 * Fix SKU 1 Duplicate
 * SKU "1" duplicate mahsulotlarni tuzatish
 */
...
async function fixSku1Duplicate() {
  ...
  try {
    ...
    // SKU "1" li mahsulotlarni topish
    const duplicateProducts = await collection.find({ sku: "1" }).toArray();
    
    console.log(`Found ${duplicateProducts.length} products with SKU "1":`);
    
    if (duplicateProducts.length > 1) {
      console.log('\n🔧 Fixing duplicate SKUs...');
      
      // Eng eski mahsulotni saqlash, qolganlarini SKU ni o'zgartirish
      ...
      console.log(`\n✅ Keeping oldest product: ${keepProduct.name} (Stock: ${keepProduct.stock})`);
      ...
      for (let i = 0; i < duplicatesToFix.length; i++) {
        ...
        const newSku = `1-${i + 1}`; // 1-1, 1-2, etc.
        ...
        console.log(`   ✅ Changed SKU: ${duplicate._id} -> "${newSku}" (Stock: ${duplicate.stock})`);
      }
    ...
    // Final verification
    console.log('\n🔍 Final verification...');
    const finalCheck = await collection.find({ sku: "1" }).toArray();
    console.log(`Products with SKU "1": ${finalCheck.length}`);
    
    if (finalCheck.length === 1) {
      const product = finalCheck[0];
      console.log(`✅ Single product with SKU "1":`);
