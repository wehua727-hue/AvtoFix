...
async function fixDuplicateSkusDirect() {
  // MongoDB connection - to'g'ridan-to'g'ri URI
  ...
  try {
    ...
    console.log('=== DUPLICATE SKU TUZATISH ===');
    ...
    if (duplicates.length === 0) {
      ...
      if (missingSkus.length > 0) {
        ...
        // Yo'qolgan SKU larni yaratish taklifi
        console.log('\n=== YO\'QOLGAN SKU LARNI YARATISH ===');
        for (const missingSku of missingSkus) {
          // Eng katta SKU ni topish
          const maxSku = Math.max(...Array.from(usedSkus).map(s => parseInt(s)).filter(n => !isNaN(n)));
          const nextSku = maxSku + 1;
          
          console.log(`SKU ${missingSku} o'rniga SKU ${nextSku} ni ishlatishingiz mumkin`);
        }
      }
    ...
    // 4. Duplicate larni ko'rsatish
    console.log('\n=== TOPILGAN DUPLICATE LAR ===');
    duplicates.forEach(dup => {
      console.log(`\nSKU "${dup.sku}" (${dup.products.length} marta):`);
      dup.products.forEach((prod, index) => {
        console.log(`  ${index + 1}. "${prod.name}" (${prod.type})`);
      });
    });
...
fixDuplicateSkusDirect().catch(console.error);