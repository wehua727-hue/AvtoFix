    ...
    for (const sale of sales) {
      try {
        ...
        for (const item of sale.items) {
          const stockChange = sale.saleType === 'refund' 
            ? item.quantity 
            : -item.quantity;
          ...
          if (variantMatch) {
            // This is a variant - update variant stock
            const originalProductId = variantMatch[1];
            const variantIndex = parseInt(variantMatch[2], 10);
            
            console.log(`[Offline Sync] Updating variant stock: ${originalProductId}[${variantIndex}], change: ${stockChange}, quantity: ${item.quantity}`);
            
            // Получаем текущий stock варианта
            const product = await ProductModel.findById(originalProductId);
            if (product && product.variantSummaries && product.variantSummaries[variantIndex]) {
              ...
              console.log(`[Offline Sync] Variant stock: ${currentStock} -> ${newStock} (change: ${stockChange})`);
              
              // Используем универсальную функцию обновления (передаем уже загруженный продукт для оптимизации)
              await updateStock(originalProductId, variantIndex, newStock, product);
            } else {
              console.warn(`[Offline Sync] Variant not found: ${originalProductId}[${variantIndex}]`);
            }
          } else {
            // Regular product - update main stock
            // Получаем текущий stock продукта
            const product = await ProductModel.findById(item.productId);
            if (product) {
              const currentStock = product.stock || 0;
              const newStock = Math.max(0, currentStock + stockChange);
              
              console.log(`[Offline Sync] Product stock: ${currentStock} -> ${newStock} (change: ${stockChange})`);
              
              // Используем универсальную функцию обновления (передаем уже загруженный продукт для оптимизации)
              await updateStock(item.productId, null, newStock, product);
              
              // Stock 0 bo'lsa ham mahsulot o'z joyida qoladi
              // Variant promotion QILINMAYDI - mahsulot shunchaki "Tugagan" deb ko'rsatiladi
            } else {
              console.error(`[Offline Sync] Product not found: ${item.productId}`);
            }
          }
        }
