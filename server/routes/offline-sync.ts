  ...
  try {
    ...
    for (const sale of sales) {
      try {
        ...
        // 3. Create sale record only - NO STOCK UPDATE
        // Stock yangilash FAQAT alohida API orqali amalga oshiriladi
        console.log('[Offline Sync] Creating sale record without stock update...');
        console.log('[Offline Sync] ⚠️  STOCK UPDATE DISABLED - will be handled by separate API call');
        
        // Sale record yaratish (stock yangilamasdan)
        await OfflineSaleModel.create({
          offlineId: sale.id,
          recipientNumber: sale.recipientNumber,
          items: sale.items,
        ...
        console.log('[Offline Sync] ✅ Sale record created without stock update:', sale.id);
