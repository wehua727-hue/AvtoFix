...
async function deleteUserData(userId: string, deleteProducts: boolean = false) {
  const conn = await connectMongo();
  ...
  try {
    // 1. Mahsulotlarni o'chirish o'rniga, userId ni null qilish (mahsulotlarni saqlash)
    // MUHIM: Foydalanuvchi o'chirilganda, uning mahsulotlari o'chirilmasligi kerak
    // LEKIN agar deleteProducts=true bo'lsa, mahsulotlarni o'chirish
    if (deleteProducts) {
      const deleteResult = await ProductModel.deleteMany({ userId });
      deletedProducts.deletedCount = deleteResult.deletedCount;
      ...
      deletedProducts.deletedCount = updateResult.modifiedCount;
      console.log(`[deleteUserData] Cleared userId for ${deletedProducts.deletedCount} products for user ${userId}`);
    }
  } catch (err) {
...
export async function handleUserDelete(req: Request, res: Response) {
  try {
    ...
    if (user.role === 'egasi') {
      ...
      // Удаляем данные каждого помощника (deleteProducts=true - o'chirish)
      for (const helper of helpers) {
        const helperData = await deleteUserData(helper._id.toString(), true);
    ...
    // Удаляем данные самого пользователя (deleteProducts=true - o'chirish)
    const userData = await deleteUserData(id, true);
