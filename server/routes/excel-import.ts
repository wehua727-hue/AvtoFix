...
export const handleExcelImport: RequestHandler = async (req, res) => {
  try {
    ...
    // Duplikat tekshirishni o'chiramiz - faqat yangi mahsulotlarni qo'shamiz
    // Avtomatik SKU raqamlari bilan duplikat tekshirish noto'g'ri ishlayapti
    ...
    for (const [, productRows] of groupedMap) {
      try {
        ...
        // SKU - avtomatik ketma-ket raqam (Nomerofka)
        const productSku = String(globalSku);
        globalSku++;
        
        // Excel dan kod va katalog
        const productCode = mainRow.code || ''; // Mahsulot kodi
        const productCatalogNumber = mainRow.catalogNumber || ''; // Katalog raqami
        ...
        if (productRows.length > 1) {
          for (let i = 1; i < productRows.length; i++) {
            ...
            // Excel dan kod va katalog
            const variantCode = variantRow.code || ''; // Xil kodi
            const variantCatalogNumber = variantRow.catalogNumber || ''; // Katalog raqami
            
            variantSummaries.push({
              name: variantRow.name,
              sku: variantSku,
              code: variantCode || undefined, // Excel dan kelgan kod
              catalogNumber: variantCatalogNumber || undefined, // Excel dan kelgan katalog
        ...
        productsToInsert.push({
          ...
          code: productCode || undefined, // Excel dan kelgan kod
          catalogNumber: productCatalogNumber || undefined, // Excel dan kelgan katalog
          basePrice: basePrice,
