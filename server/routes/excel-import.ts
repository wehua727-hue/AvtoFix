...
interface ColumnMapping {
  ...
  basePrice: number; // Asl narx (yangi)
...
const HEADER_KEYWORDS = {
  ...
  price: ['цена', 'narx', 'price', 'стоимость', 'аsl нархи', 'asl narxi', 'base price', 'базовая цена'],
  basePrice: [], // Bo'sh - ishlatilmaydi
...
export const handleExcelImport: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    interface ParsedRow {
      ...
      price: number;
      basePrice: number; // Asl narx qo'shildi
      stock: number;
      category: string;
    ...
    for (let i = startRow; i < rawData.length; i++) {
      ...
      let catalogNumber = '';
      let price = 0;
      let basePrice = 0; // Asl narx qo'shildi
      ...
      // Narxi (aslida asl narx - basePrice sifatida saqlanadi)
      if (columnMap.price >= 0 && row[columnMap.price] !== undefined && row[columnMap.price] !== null) {
        ...
        if (!isNaN(parsed) && parsed > 0) {
          basePrice = parsed; // MUHIM: price ustunidan olingan qiymat basePrice ga saqlanadi
          console.log('[Excel Import] Row', i, '- price column:', columnMap.price, ', raw value:', row[columnMap.price], ', saved as basePrice:', basePrice);
        }
      }
      
      // Asl narx (basePrice) - YANGI (hozir ishlatilmaydi)
      if (columnMap.basePrice >= 0 && row[columnMap.basePrice] !== undefined && row[columnMap.basePrice] !== null) {
        const parsed = parseFloat(String(row[columnMap.basePrice]).replace(/[^0-9.]/g, ''));
        if (!isNaN(parsed) && parsed > 0) {
          basePrice = parsed;
          console.log('[Excel Import] Row', i, '- basePrice column:', columnMap.basePrice, ', raw value:', row[columnMap.basePrice], ', parsed basePrice:', basePrice);
        }
      }
      ...
      rows.push({ name, sku, code, catalogNumber, price, basePrice, stock, category, barcodeId, multiplier: multiplierValue });
    }
    ...
    // DEBUG: Guruhlarni ko'rsatish
    console.log('[Excel Import] ===== GROUPS DEBUG =====');
    for (const [key, products] of groupedMap) {
      console.log(`[Excel Import] Group "${key}": ${products.length} products`);
      products.forEach((p, i) => {
        console.log(`[Excel Import]   ${i+1}. ${p.name}`);
      });
    }
    console.log('[Excel Import] ===== END GROUPS DEBUG =====');

    // MAJBURIY GURUHLASH: Agar bir guruhda 1 ta mahsulot bo'lsa ham, boshqa guruhlar bilan birlashtirish
    console.log('[Excel Import] ===== FORCED GROUPING =====');
    const forcedGroupedMap = new Map<string, ParsedRow[]>();
    
    for (const row of rows) {
      // Birinchi SO'ZNI olish (faqat birinchi so'z)
      const words = row.name.split(/\s+/);
      const firstWord = words[0]?.toLowerCase() || '';
      
      if (firstWord) {
        const existing = forcedGroupedMap.get(firstWord);
        if (existing) {
          existing.push(row);
        } else {
          forcedGroupedMap.set(firstWord, [row]);
        }
      }
    }
    
    console.log('[Excel Import] Forced groups (by first WORD):');
    for (const [key, products] of forcedGroupedMap) {
      console.log(`[Excel Import] Forced Group "${key}": ${products.length} products`);
    ...
    // Agar birinchi ikki so'z bo'yicha guruhlash kam natija bersa, birinchi so'z bo'yicha ishlatish
    let finalGroupedMap = groupedMap;
    const multiProductGroups = Array.from(groupedMap.values()).filter(group => group.length > 1).length;
    const forcedMultiProductGroups = Array.from(forcedGroupedMap.values()).filter(group => group.length > 1).length;
    
    if (forcedMultiProductGroups > multiProductGroups) {
      console.log('[Excel Import] Using FORCED grouping (first word) because it found more groups');
      finalGroupedMap = forcedGroupedMap;
    } else {
      console.log('[Excel Import] Using NORMAL grouping (first two words)');
    }
    console.log('[Excel Import] ===== END FORCED GROUPING =====');
    ...
    for (const [firstTwoWords, productRows] of finalGroupedMap) {
      try {
        ...
        if (existingProduct) {
          ...
          for (const row of productRows) {
            ...
            // MUHIM: Variant uchun asl narx Excel dan olinadi, sotish narxi hisoblanadi
            ...
            const variantBasePrice = row.basePrice || 0; // Excel dan olingan asl narx
        ...
        // MUHIM: Asl narx Excel dan olinadi, sotish narxi hisoblanadi
        const basePrice = mainRow.basePrice || 0; // Excel dan olingan asl narx
        ...
        if (productRows.length > 1) {
          for (let i = 1; i < productRows.length; i++) {
            ...
            const variantBasePrice = variantRow.basePrice || basePrice; // Excel dan olingan asl narx
            const variantSellingPrice = variantBasePrice > 0 
              ? Math.round((variantBasePrice * (1 + variantMultiplierDecimal)) * 100) / 100 
              : sellingPrice;
...
/**
 * POST /api/excel-import/diagnostic
 * Excel import qilingan mahsulotlarni tekshirish va variant munosabatlarini tuzatish
 */
export const handleExcelImportDiagnostic: RequestHandler = async (req, res) => {
  try {
    const conn = await connectMongo();
    if (!conn || !conn.db) {
      return res.status(500).json({ success: false, error: "Database not available" });
    ...
    console.log('[Excel Diagnostic] Starting diagnostic for user:', userId);
    ...
    console.log('[Excel Diagnostic] Found Excel products:', excelProducts.length);
    ...
    for (const product of excelProducts) {
      ...
      if (existing) {
        ...
        groupedMap.set(firstTwoWords, [product]);
      }
    }

    console.log('[Excel Diagnostic] Unique groups (by first two words):', groupedMap.size);

    const diagnosticResults = [];
    let fixedCount = 0;

    // Har bir guruhni tekshirish
    for (const [firstTwoWords, products] of groupedMap) {
      if (products.length > 1) {
        console.log(`[Excel Diagnostic] Group "${firstTwoWords}" has ${products.length} products - should be variants`);
        ...
        diagnosticResults.push({
          ...
          variants: variantProducts.map(v => v.name),
          fixed: true
        });

        fixedCount++;
        console.log(`[Excel Diagnostic] Fixed group "${firstTwoWords}": ${mainProduct.name} + ${variantProducts.length} variants`);
      } else {
        diagnosticResults.push({
          group: firstTwoWords,
          mainProduct: products[0].name,
          variants: [],
          fixed: false
        });
      }
    ...
    console.log('[Excel Diagnostic] Fixed groups:', fixedCount);

    return res.json({
      success: true,
      message: `${fixedCount} ta guruh tuzatildi`,
      totalGroups: groupedMap.size,
      fixedGroups: fixedCount,
      results: diagnosticResults
    });

  } catch (error: any) {
    console.error("[Excel Diagnostic] Error:", error);
    return res.status(500).json({ 
      success: false, 
      error: error.message || "Diagnostic xatosi" 
    });
  }
};