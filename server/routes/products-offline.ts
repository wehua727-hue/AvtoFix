...
export const handleProductDelete: RequestHandler = async (req, res) => {
  try {
    ...
    // MUHIM: Agar mahsulotda xillar (variantlar) bo'lsa, birinchi xilni ota mahsulot qilish
    ...
    if (productVariants.length > 0) {
      ...
      // Birinchi xilni topish (MongoDB versiyasidagi kabi)
      const firstVariant = productVariants[0];
      const remainingVariants = productVariants.slice(1);
      
      console.log('[Products DELETE] First variant:', firstVariant?.sku, firstVariant?.name);
      console.log('[Products DELETE] Remaining variants:', remainingVariants.length);
      
      if (firstVariant) {
        // Eski mahsulotni birinchi xil bilan yangilash (yangi mahsulot yaratish emas!)
        const updateData = {
          name: firstVariant.name, // Variant nomini saqlab qolish
          ...
          // Qolgan xillarni variantSummaries ga qo'shish (birinchi xildan boshqa)
          variantSummaries: remainingVariants,
          updatedAt: new Date()
        };
        
        console.log('[Products DELETE] Updating main product with first variant, remaining variants:', remainingVariants.length);
        
        // Eski mahsulotni yangilash (o'chirish emas!)
        const updated = updateDocument(PRODUCTS_COLLECTION, id, updateData);
        
        if (!updated) {
          return res.status(500).json({ success: false, error: "Failed to update product with variant" });
        }

        // Save to history - variant promoted
        try {
          addDocument(PRODUCT_HISTORY_COLLECTION, {
            userId: userId || 'system',
            type: 'delete',
            ...
            sku: product.sku || '',
            stock: product.stock || 0,
            message: `Ota mahsulot o'chirildi, birinchi xil ota bo'ldi: ${product.name} → ${firstVariant.name}`,
    ...
    console.log('[Products DELETE] Product has no variants - deleting normally');
    ...
    // Save to history - normal deletion
    try {
      addDocument(PRODUCT_HISTORY_COLLECTION, {
        userId: userId || 'system',
        type: 'delete',
