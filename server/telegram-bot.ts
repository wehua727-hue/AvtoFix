...
export function initTelegramBot() {
  ...
  if (bot) {
    try {
      bot.stopPolling();
      console.log('[Telegram Bot] Stopped existing bot instance');
    } catch (error) {
      console.warn('[Telegram Bot] Error stopping existing bot:', error);
    }
    bot = null;
  }
...
export function stopTelegramBot() {
  if (bot) {
    try {
      ...
      console.error('[Telegram Bot] Error stopping bot:', error);
    }
    bot = null;
  }
}
